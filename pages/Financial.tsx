          ...
          <section className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
              ...
              {isPaymentMethodsOpen && (
                  <div className="p-6 border-t border-slate-100 animate-fade-in"><div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">{PAYMENT_METHODS_LIST.map(method => (
                      ...
                      <div className="flex justify-end">{saveMsg && <span className="text-green-600 font-medium text-sm mr-4 animate-fade-in">{saveMsg}</span>}<button onClick={async () => { if (currentCompany && !isReadOnly) { const result = await updateCompany(currentCompany.id, { paymentMethods: selectedPaymentMethods }); if (!result.success) { showAlert(result.error ?? 'Erro de sistema. Tente novamente.', { variant: 'danger' }); return; } setSaveMsg('Salvo!'); setTimeout(() => setSaveMsg(''), 2000); } }} disabled={isReadOnly} className={`px-4 py-2 rounded-lg flex items-center gap-2 text-sm font-bold ${isReadOnly ? 'bg-slate-300 text-white cursor-not-allowed' : 'bg-slate-800 text-white'}`}><Save className="w-4 h-4" /> Salvar</button></div></div>)}</section>)}
