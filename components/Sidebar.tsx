...
import { LayoutDashboard, Users, Calendar, DollarSign, Settings, LogOut, Syringe, Briefcase, Link as LinkIcon, Megaphone, LifeBuoy, Tag, BellRing, AlertTriangle, Save, Clock, BarChart3, Zap, Package, History } from 'lucide-react';
...
const Sidebar: React.FC = () => {
  ...
  const filteredItems = navItems.filter(item => {
    ...
    if (item.module) {
        // Se for Owner do SaaS, vê tudo. Se for usuário da clínica, checa o plano.
  ...
  return (
    <>
        <aside 
            className={`w-64 h-screen fixed left-0 top-0 flex flex-col z-50 text-white shadow-2xl transition-colors duration-300 ${!isPatient ? 'bg-secondary-900' : ''}`}
            style={sidebarStyle}
        >
        {/* Header com Logo e Perfil do Usuário */}
        <div className={`p-6 border-b shrink-0 ${isPatient ? 'border-white/10' : 'border-secondary-800'}`}>
            <div className="flex items-center gap-3 mb-6">
                ...
                <div className="flex flex-col overflow-hidden">
                    <span className={`text-xl font-serif font-bold tracking-wide truncate ${isPatient ? 'text-white' : 'text-primary-100'}`}>
                        {brandingName}
                    </span>
            ...
            <div className={`flex items-center gap-3 px-3 py-3 rounded-xl border ${isPatient ? 'bg-black/10 border-white/10' : 'bg-secondary-800/50 border-secondary-700/50'}`}>
                <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold shrink-0 shadow-inner ${isPatient ? 'bg-white' : 'bg-secondary-800 border border-secondary-600 text-primary-400'}`} style={isPatient ? { color: patientPrimaryColor } : {}}>
                ...
                <div className="overflow-hidden">
                    ...
                    {!isPatient && !isPatient && (
                        <p className="text-[10px] text-secondary-400 truncate">{currentCompany?.name}</p>
                    )}
                    <div className="flex items-center gap-2 mt-0.5">
                        {/* Badge do Plano */}
                        {showPlan && (
                            <span className={`text-[10px] px-1.5 py-0.5 rounded font-bold uppercase tracking-wide ${getPlanBadgeColor(currentCompany?.plan || (user?.role === UserRole.OWNER ? 'clinic' : 'free'))}`}>
        ...
        <nav className="flex-1 p-4 space-y-2 mt-2 overflow-y-auto custom-scrollbar">
            ...
            >
                <item.icon className={`w-5 h-5 ${isActive(item.path) ? 'currentColor' : (isPatient ? 'text-white/70 group-hover:text-white' : 'text-secondary-500 group-hover:text-primary-300')}`} />
        ...
        <div className={`p-4 border-t ${isPatient ? 'border-white/10 bg-black/10' : 'border-secondary-800 bg-secondary-900/50'}`}>
